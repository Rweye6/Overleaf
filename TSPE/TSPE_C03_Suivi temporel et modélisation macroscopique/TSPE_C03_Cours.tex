...
\draw (3.2,0) node[below]{\Large Temps t (s)};
...
\draw (0,3.2) node[right]{\Large Quantités de matière n (mol)};
...
\draw (2.5,0) node[below]{\large Concentration [X] (\unit{\mole\per\liter})};
\draw (0,3.2) node[right]{\large Vitesse volumique $v_x$ (\unit{\mole\per\liter\per\second})};
...
Modélisation microscopique cinétique

Modéliser une réaction chimique à l'échelle microscopique permet de mieux comprendre et expliquer l'influence des facteurs cinétiques.
%\newpage
\section{Mécanismes réactionnels}
	\subsection{Chocs efficaces}
Une réaction chimique ne peut avoir lieu que d'il y a contact entre deux entités chimiques. Lors de cette rencontre, la réaction chimique peut être décrite comme une réorganisation des atomes ou des charges électriques entre les deux entités. Cela implique des ruptures de liaisons covalentes ou des arrachages d'électrons avant la formation de nouvelles liaisons ou la récupération d'électrons.\\
...
\subfloat[\textbf{Attaque d'un doublet non liant}]{
	...
	\centering
    \schemestart
 	\chemfig{H-C(-[:90]H)(-[:-90]H)-@{dnl}\charge{-120[anchor=180+\chargeangle]=$\delta^-$,35=\|,125=\|}{O}-[:-35]\charge{-60[anchor=180+\chargeangle]=$\delta^+$}{H}} 
