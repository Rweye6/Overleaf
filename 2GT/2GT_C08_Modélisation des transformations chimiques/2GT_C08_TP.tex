...
\newcommand{\chtitle}{Liquide magique}
\newcommand{\dtype}{TP}
...
\section{Nombre de molécules et de moles}
	...
	\hline
	\end{tabular}
	\subsection*{Document 2 : Le glucose}
	\begin{tabular}{>{\centering}m{6cm}>{\centering \arraybackslash}m{6cm}}
	Modèle moléculaire du glucose : & Formule brute : \\
	\includegraphics[scale=.2]{molecular_glucose_render_1_06.jpg}&$C_{6}H_{12}O_{6}$ \\
	\end{tabular}
	...
	\begin{enumerate}
		\item Calculer la masse $m_{glc}$ d'une molécule de glucose en détaillant la démarche.\\
		\begin{tikzpicture}
		\draw (0, 0) -- (18, 0) -- (18, 5) -- (0, 5) -- (0, 0);
		\end{tikzpicture}
		\item Grâce à une balance électronique, déterminer le nombre $N$ de molécules de glucose dans une masse de 5 g.\\
		\begin{tikzpicture}
		\draw (0, 0) -- (18, 0) -- (18, 5) -- (0, 5) -- (0, 0);
		\end{tikzpicture}
		\item Combien de moles $n$ de glucose contient une spatule de sucre ?\\
		\begin{tikzpicture}
		\draw (0, 0) -- (18, 0) -- (18, 5) -- (0, 5) -- (0, 0);
		\end{tikzpicture}
	\end{enumerate}
...
\begin{enumerate}
	...
	\item Prélever à l'aide d'une spatule, $9,6 \times 10^{21}$ molécules de glucose en poudre et les introduire dans l'erlenmeyer.
