...
\begin{table}
	\begin{tabular}{c>{\centering}m{3cm}c}
		\includegraphics[scale=.15]{Ressources/Voix.jpg}&Quel est le point commun entre ces images ?&\includegraphics[scale=.165]{Ressources/unnamed.jpg}\\
		\includegraphics[scale=.2]{Ressources/violin.jpg}&\only<2>{\textbf{Le son}}&\includegraphics[scale=.7]{Ressources/eau.jpg}\\
	\end{tabular}
\end{table}
\end{frame}
...
\begin{frame}
	...
	\begin{minipage}{.48\linewidth}
		...
		\draw[color = gray!30] (0,-1) grid[step=.1](1,1);
		\draw [domain=0:1,samples = 200, color = blue] plot(\x,{.85*sin(12.6*\x r)});
		\draw[->] (0,0) -- (1.1,0);
		\draw (1.1,0) node[below right] {\tiny $t (s)$};
		\draw[->] (0,-1) -- (0,1);
		...
		\foreach \i in {.2,.4,...,1}{
			\draw (\i,-.025)--(\i,.025);
			\node[below] (n2) at (\i,-.05){\tiny \pgfmathprintnumber[precision=2]{\i}};
		}
		%\foreach \d in {0,1,...,6} {
...
\begin{frame}
	...
	\begin{center}
		\begin{tikzpicture}
		\draw[dashed] (0,0)--(5,0);
		\draw[very thick, green!70!blue] plot[domain=0:5,samples=200,color=blue] (\x,{0.75*sin(6.28*\x r) + 0.5*sin(2*6.28*\x r)});
		\draw[<->,thick] (0,-1.5)--(1,-1.5);
	\end{tikzpicture}\\\vspace{1em}
	\begin{tikzpicture}
		\draw[dashed] (0,0)--(5,0);
		\draw[very thick, purple!70!blue] plot[domain=0:5,samples=200,color=blue] (\x,{0.75*sin(6.28*\x r) + 0.5*sin(3*6.28*\x r)});
	\end{tikzpicture}
	\end{center}
