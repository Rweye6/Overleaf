...
\begin{document}
    ...
    \noindent \begin{minipage}{.58\linewidth}
...
\end{minipage}
    \begin{minipage}{.35\linewidth}
        \centering
        \includegraphics[width=.5\linewidth]{Ressources/1SPE_C15_Cours_Bateau.png}
    \end{minipage}
...
\end{cours}
On peut identifier ce type de fonction à partir du graphique comportant une alternance de "vagues" positives et négatives de mêmes amplitudes.\\
...
Un diapason permet de générer un son quasiment sinusoidal. L'enregistrement réalisé à l'aide d'un microphone donne la courbe ci-contre.\\

\textbf{Célérité du son dans l'air :} $v_{air} = \SI{340}{\meter\per\second}$\\

\begin{tabular}{|c|c|c|c|c|c|c|c|}
    \hline
    \textbf{Note} & Do3 & Ré3 & Mi3 & Fa3 & Sol3 & La3 & Si3\\
    \hline
    $f(\unit{\hertz})$ & 262 & 294 & 330 & 349 & 392 & 440 & 494\\
    \hline
\end{tabular}
...
\begin{minipage}{.45\linewidth}
    ...
    \begin{tikzpicture}[scale=1.5, transform shape]
        ...
        \draw[-Latex] (0,0) -- (4.7,0);
        \draw[domain = 0:4.5,samples = 200,thick,red] plot (\x,{sin(155*\x)});
        \foreach \k in {0,.5,...,4.5}{
            \draw[gray] (\k,-1)--(\k,1);
            \draw[thick] (\k,-.05)--(\k,.05);
            \node[below] at (\k,0) {\tiny\k};
        }
        \foreach \k in {-1,-0.75,...,1}{
            \draw[gray] (0,\k)--(4.5,\k);
            \draw[thick] (-.05,\k)--(.05,\k);
            \node[left] at (0,\k) {\tiny\k};
        }
        \node[above] at(0,1){\tiny $U(V)$};
        \node[below] at(4.5,-1){\tiny $t(ms)$};
        
    \end{tikzpicture}
\end{minipage}
\begin{enumerate}
    \item Déterminer la période depuis la fréquence du son émis par le diapason.
    \item À quelle note correspond sa hauteur ?
